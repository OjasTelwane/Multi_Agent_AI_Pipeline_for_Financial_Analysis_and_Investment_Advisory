...
def run_single_interactive(data_dir: Path, output_dir: Path) -> int:
    ...
    results, cerebro = run_backtest(
        ohlc_data, AgentStrategy, "Agent", signals_df=signals_df, printlog=printlog
    )
    ...
    for m in metrics:
        pv, bv = results[m], buyhold_results[m]
        diff = pv - bv
    ...
    rel = results["Cumulative Return [%]"] - buyhold_results["Cumulative Return [%]"]
    if rel > 0:
        print(f"\nAgent OUTPERFORMED Buy & Hold by {rel:+.2f}%!")
    else:
        print(f"\nAgent underperformed Buy & Hold by {abs(rel):.2f}%")

    chart_path = plot_single_stock(symbol, cerebro, buyhold_cerebro, str(output_dir))
...
def run_multi_interactive(data_dir: Path, output_dir: Path) -> int:
    ...
    for symbol in symbols:
        ...
        if ohlc_data is None or signals_df is None:
            ...
            continue
        try:
            agent_results, agent_cerebro = run_backtest(
                ohlc_data, AgentStrategy, f"{symbol} PrimoAgent", signals_df=signals_df, printlog=printlog
            )
            buyhold_results, buyhold_cerebro = run_backtest(
                ohlc_data, BuyAndHoldStrategy, f"{symbol} Buy & Hold"
            )
            all_results[symbol] = {"primo": agent_results, "buyhold": buyhold_results}

            # individual chart
            _ = plot_single_stock(symbol, agent_cerebro, buyhold_cerebro, str(output_dir), f"backtest_results_{symbol}.png")

            # quick comparison
            agent_return = agent_results["Cumulative Return [%]"]
            buyhold_return = buyhold_results["Cumulative Return [%]"]
            rel = agent_return - buyhold_return
            if rel > 0:
                print(f"{symbol}: PrimoAgent +{rel:.2f}% ( {agent_return:.2f}% vs {buyhold_return:.2f}% )")
            else:
                print(f"{symbol}: PrimoAgent -{abs(rel):.2f}% ( {agent_return:.2f}% vs {buyhold_return:.2f}% )")
