...
def plot_single_stock(symbol: str, cerebro, buyhold_cerebro, output_dir: str, filename: str | None = None) -> Path:
    """Create single-stock portfolio comparison chart and save it."""
    dates, prices = _extract_dates_prices(cerebro)

    strategy = cerebro.runstrats[0][0]
    portfolio = getattr(strategy, "portfolio_values", [])
    ...
    if len(portfolio) != len(dates):
        if len(portfolio) < len(dates):
            last_value = portfolio[-1] if portfolio else DEFAULT_CASH
            portfolio.extend([last_value] * (len(dates) - len(portfolio)))
        else:
            portfolio = portfolio[: len(dates)]

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(15, 10), gridspec_kw={"height_ratios": [2, 1]})

    ax1.plot(dates, portfolio, color="blue", linewidth=2, label="Agent Portfolio")
    ax1.plot(dates, buyhold_portfolio, color="red", linewidth=2, label="Buy & Hold Portfolio")
    ax1.set_ylabel("Portfolio Value ($)")

    signals_df = getattr(strategy, "signals_df", None)
...
def plot_returns_bar_chart(all_results: Dict[str, Dict[str, Any]], save_path: Path) -> None:
    ...
    returns = [all_results[s][""]["Cumulative Return [%]"] for s in symbols]
    buyhold_returns = [all_results[s]["buyhold"]["Cumulative Return [%]"] for s in symbols]

    x = range(len(symbols))
    width = 0.35
    bars1 = ax.bar([i - width / 2 for i in x], returns, width, label="Agent", color="#1f77b4", alpha=0.8)
